...
open class Event<Input> {
    ...
    /// - Warning: Внутри одного файла для одного события нельзя добавлять несколько слушателей без явного указания ключа. В противном случае события будут перезаписаны.
    ...
    ///   - listener: Слушатель.
    open func add(key: String = #file, _ listener: @escaping Closure) {
